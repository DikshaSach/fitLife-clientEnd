 ...
 constructor(props) {
    ...
    render(){
       ...
       const listView = strengthExercise.map((itm,index)=>
        ...
        <img className="muscle-icon" src={MuscleIcon} alt="muscle icon left" height="50" width="auto"/>
        <img className="muscle-icon-right" src={MuscleIconRight} alt="muscle icon right" height="50" width="auto"/>
        </div>
       ...
       <span className="strength-exercise-name">{itm.name} </span>
        ...
        return <div className="display-event-container">
           ...
           <button className="exercise-edit-bttn" onClick={() => this.clickedEdit()}>Edit</button>
            <button   className="exercise-delete-bttn" onClick={()=> this.clickedDelete()}>Delete</button>
            </div>
            <h1 className="exercise-title">{title}</h1>
            <h1 className="exercise-date">Date: {startdateMonth}/{startdateDate}/{startdateYear}</h1>
            {this.props.eventsData.singleEvent["0"].time ? <h1 className="exercise-time-spent">Time Spent: {time} </h1> : null}
